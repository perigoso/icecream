/*
 *  icecream -- A small open source driver program for the CP2130 to program ice40 fpga's.
 */
...
static void set_cs_creset(int cs_b, int creset_b)
{
	...
	if(creset_b)
    {
		// ADBUS7 (GPIOL3)
		//gpio |= 0x80;
        gpio |= CP2130_GPIO3_MSK;
	}

    cp2130_set_gpio_values(cpdev, gpio, CP2130_GPIO0_MSK | CP2130_GPIO3_MSK);
}
...
static bool get_cdone(void)
{
	// ADBUS6 (GPIOL2)
    uint16_t gpio = 0;
    cp2130_get_gpio_values(cpdev, &gpio);
	//return (mpsse_readb_low() & 0x40) != 0;
    return (gpio & CP2130_GPIO2_MSK) != 0;
}
...
static void flash_read_id()
{
    ...
    cp2130_spi_transfer(cpdev, data, len);

	if(data[4] == 0xFF)
		fprintf(stderr, "Extended Device String Length is 0xFF, this is likely a read error. Ignoring...\n");
	else
    {
        ...
        {
			len += data[4];
			cp2130_spi_read(cpdev, data + 5, len - 5);
		}
	}
...
static void flash_power_up()
{
	...
	flash_chip_select();
	cp2130_spi_transfer(cpdev, data_rpd, 1);
	flash_chip_deselect();
}
...
static void flash_power_down()
{
	...
	flash_chip_select();
	cp2130_spi_transfer(cpdev, data, 1);
	flash_chip_deselect();
}
...
static uint8_t flash_read_status()
{
	...
	flash_chip_select();
	cp2130_spi_transfer(cpdev, data, 2);
	flash_chip_deselect();
...
static void flash_64kB_sector_erase(int addr)
{
	...
	flash_chip_select();
	cp2130_spi_transfer(cpdev, command, 4);
	flash_chip_deselect();
}
...
static void flash_prog(int addr, uint8_t *data, int n)
{
	...
	flash_chip_select();
	cp2130_spi_write(cpdev, command, 4);
	cp2130_spi_write(cpdev, data, n);
	flash_chip_deselect();
...
{
	fprintf(stderr, "A small open source driver program for the CP2130 to program ice40 fpga's.\n");
	...
	fprintf(stderr, "  -s                    slow SPI (375kHz instead of 12MHz)\n");
	...
	fprintf(stderr, "  https://github.com/gimbas/icecream/issues\n");
}
...
int main(int argc, char **argv)
{
	...
	int opt;
	char *endptr;
	while ((opt = getopt_long(argc, argv, "d:rR:e:o:cbnStvsp", long_options, NULL)) != -1) {
		switch (opt) {
	...
	// ---------------------------------------------------------
	// Initialize USB connection to CP2130
	// ---------------------------------------------------------
    ...
    {
        int dev_vid, dev_pid;
        sscanf(devstr, "i:0x%X:0x%X", &dev_vid, &dev_pid);

        cpdev = cp2130_init(ctx, (uint16_t)(dev_vid & 0xFFFF), (uint16_t)(dev_pid & 0xFFFF));
        if(!cpdev)
        {
            fprintf(stderr, "failed to initialize device with vid:%04X pid:%04X.\n", dev_vid, dev_pid);
            return -1;
        }
    }
    else
    {
        cpdev = cp2130_init(ctx, CP2130_DEFAULT_VID, CP2130_DEFAULT_PID);
        if(!cpdev)
        {
    ...
    {
        uint8_t major, minor;
        cp2130_get_version(cpdev, &major, &minor);
        fprintf(stderr, "Version: %0d.%0d\n\r", major, minor);

        uint8_t str[63];
        memset(str, 0x00, 63);
        cp2130_get_manufacturer_string(cpdev, str);
        fprintf(stderr, "Manufacturer: %s\n\r", str);
        memset(str, 0x00, 63);
        cp2130_get_prod_string(cpdev, str);
        fprintf(stderr, "Product: %s\n\r", str);
        memset(str, 0x00, 63);
        cp2130_get_serial(cpdev, str);
        fprintf(stderr, "Serial: %s", str);
    }

    cp2130_set_gpio_mode_level(cpdev, CP2130_GPIO0, CP2130_GPIO_OUT_PP, CP2130_GPIO_LOW);
    cp2130_set_gpio_mode_level(cpdev, CP2130_GPIO2, CP2130_GPIO_OUT_PP, CP2130_GPIO_LOW);
    cp2130_set_gpio_mode_level(cpdev, CP2130_GPIO3, CP2130_GPIO_OUT_PP, CP2130_GPIO_LOW);

    cp2130_set_gpio_cs(cpdev, CP2130_CS_CH0, CP2130_CS_MD_EN_DIS_OTHERS);

    if(slow_clock)
    {
        cp2130_set_spi_word(cpdev, CP2130_SPI_CH0, CP2130_SPI_WRD_CS_MODE_PP | CP2130_SPI_WRD_CLK_375K);
    }
    else
    {
        cp2130_set_spi_word(cpdev, CP2130_SPI_CH0, CP2130_SPI_WRD_CS_MODE_PP | CP2130_SPI_WRD_CLK_12M);
    }
	...
	else if(prog_sram)
	{
		...
		while(1)
        {
			...
			cp2130_spi_write(cpdev, buffer, rc);
		}

        uint8_t dummy[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
		cp2130_spi_write(cpdev, dummy, 6);

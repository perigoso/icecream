...
cp2130_device_t *cp2130_init(libusb_context *ctx, uint16_t usVid, uint16_t usPid)
{
    if(!ctx)
    {
        fprintf(stderr, "error: invalid libUSB context.\n");
        return NULL;
    }

    fprintf(stderr, "allocating space for device...\n");
    cp2130_device_t *pCp2130_dev = (cp2130_device_t *)malloc(sizeof(cp2130_device_t));
    if(!pCp2130_dev)
    {
        fprintf(stderr, "error: device space allocation failed.\n");
        return NULL;
    }
    ...
    {
        fprintf(stderr, "error: libusb could not open device.\n");
        free(pCp2130_dev);
        return NULL;
    }

    fprintf(stderr, "checking if attached to kernel...\n");
    if(libusb_kernel_driver_active(pCp2130_dev->pDev, 0) != 0)
    {
        fprintf(stderr, "device was atached to kernel, detaching...\n");
...
void cp2130_free(cp2130_device_t *pCpDev)
{
    ...
    {
        fprintf(stderr, "reattaching kernel...\n");
...
static void cp2130_control_transfer(cp2130_device_t *pCpDev, uint8_t ubReqType, uint8_t ubRequest, uint8_t * pubData, uint16_t usLen, uint16_t usWvalue, uint16_t usWindex)
{
    ...
    if(ulR < 0)
        fprintf(stderr, "error: failed with return code %d.", ulR);
}
static void cp2130_bulk_transfer(cp2130_device_t *pCpDev, uint8_t ubEp, uint8_t * pubData, uint32_t ulLen)
{
    ...
    if(ulTransferred < ulLen)
        fprintf(stderr, "error: failed short, got %d bytes of %d requested", ulTransferred, ulLen);
}
...
void cp2130_spi_transfer(cp2130_device_t *pCpDev, uint8_t *pubData, uint32_t ulLen)
{
    if(ulLen > 120)
    {
        fprintf(stderr, "warning: cannot transfer more than 120 bytes, use write or read for longer transfers.\n");
        return;
    }

    if(!pubData)
    {
        fprintf(stderr, "error: null data buffer.\n");
        return;
    }
...
void cp2130_set_lock_byte(cp2130_device_t *pCpDev, uint16_t usLock)
{
    ...
    #else
    fprintf(stderr, "warning: OTP ROM write protect enabled, ignoring write.\n");
    #endif
}
...
void cp2130_get_manufacturer_string(cp2130_device_t *pCpDev, uint8_t *pubStr)
{
    if(!pubStr)
    {
        fprintf(stderr, "error: invalid str pointer.\n");
        return;
    }
...
void cp2130_set_manufacturer_string(cp2130_device_t *pCpDev, uint8_t *pubStr)
{
	...
	if(ubStrLen > 62)
    {
		fprintf(stderr, "warning: string too long, max 62 chars.\n");
        return;
    }
